...
Phase 3D is now specified as a continuous versioned discovery system. The committed batch vertical slice supplies reusable causal materialization, deterministic run/candidate hashes, fixed-rule scoring, complexity penalties, family collapse, and a post-activation stable-taker evaluator. Slices C2-C3 now supply the durable append-only external registry plus recurring resolved-watermark discovery, bounded multi-challenger nomination, crash resume, unchanged-version reuse, immutable run outcomes, and explicit resource budgets. The one-winner manifest orchestration remains transitional rather than the target architecture. The next work is continuous candidate-specific cohort evaluation and champion/challenger transitions; dry-run scheduling remains deferred to C6. Current ledgers have no venue-resolution rows and Phase 3 markouts remain open, so no candidate can pass to Phase 4. The named V2a pilots remain vertical controls and receive no presumption that one must become a champion.
