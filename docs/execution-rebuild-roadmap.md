...
Phase 3D is now specified as a continuous versioned discovery system. The committed batch vertical slice supplies reusable causal materialization, deterministic run/candidate hashes, fixed-rule scoring, complexity penalties, family collapse, and a post-activation stable-taker evaluator. Slice C2 now supplies the durable append-only external registry, content-addressed candidate reuse/versioning, cohort and scorecard history, lifecycle events, one-writer locking, read-only observers, and identity-only legacy import. The one-winner manifest orchestration remains transitional rather than the target architecture. The next work is the recurring resolved-data discovery orchestrator, continuous candidate-specific cohort evaluation, and champion/challenger transitions. Current ledgers have no venue-resolution rows and Phase 3 markouts remain open, so no candidate can pass to Phase 4. The named V2a pilots remain vertical controls and receive no presumption that one must become a champion.
