...
The shared weather market tape contains retained data from July 23 onward and supports exact quote-ready taker-book replay. Bounded validation run `tape-validation-20260730T183750Z-69556cbf` completed its 72-hour limit with 224.6 million events, 1,273 complete discovery refreshes, zero reconstruction errors, queue high-water 1/10,000, about 234 MiB peak recorder RSS, and projected raw growth of about 12.0 GB/day. Operational lifecycle acceptance now permits recovered gaps up to 30 seconds and excludes late/fallback-listed markets individually; decision replay remains stricter and rejects any signal whose 60-second pre-signal-through-execution window crosses a gap. Under that standard, 418 late-discovered markets were excluded, but only 4 of 33 eligible matured markets were complete: the other 29 had 36.34-41.32 second maximum recovery gaps, with 41.21-second p50 and 41.32-second p95 across eligible markets. Peak receipt lag was 10.969 seconds and is now a warning rather than a whole-run failure. Slice 2 therefore remains open on reconnect/recovery coverage rather than duration, storage, queue, or reconstruction. The tape can remove policy-specific collection bias, but it does not prove exact passive fills, settlement-aligned PnL, capacity, or funded readiness.
...
| Phase 3 shared tape | Slices 1-4 are implemented. The 72-hour bounded validation run completed with healthy duration, discovery-refresh integrity, reconstruction, queue, memory, and storage evidence, but only 4/33 eligible matured markets stayed within the approved 30-second operational recovery budget; Slice 4 also lacks one real persisted-quote join. | Reduce reconnect recovery below 30 seconds and rerun a scoped lifecycle cohort; continue to reject every decision window that intersects a gap. |
...
4. Keep bounded shared-tape validation run `tape-validation-20260730T183750Z-69556cbf` as failed-but-useful evidence: it passed duration/resource/reconstruction gates but only 4/33 eligible markets met the 30-second operational recovery budget. Diagnose reconnect recovery, rerun a scoped lifecycle cohort, and do not promote the disabled unit into an unattended production service.
