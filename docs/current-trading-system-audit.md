...
| Forecast and settlement truth | F0 reconciled the current IEM report maximum to 220/220 venue-winning buckets; the shared selector now enforces the exact cutoff on historical and forward paths; F1 added causal source vintages; and F2/F3/F4/F6 are rejected. WeatherNext remains unscored. | Backfill provenance/venue labels, continue the separate exact-12 and exact-14 F5 forward evidence clocks and predeclared ablations, and add WeatherNext only after approved access supplies provider ingestion timestamps. |
...
| Phase 3D strategy discovery | D0-D5 are accepted. The August 13 run registered two overlapping research-only US-high variants, but both fail desk qualification on three-date uncertainty, concentration, best-date dependence, market-relative calibration, and missing markouts. No scheduler is enabled. | Preserve the versions for forward attribution only. Keep D6/Phase 4 blocked until an exact candidate passes venue, markout, economics, concentration, useful-size, and explicit approval gates. |
...
### August 13 current-data net-edge tournament

The current-watermark tournament ended with no demonstrated deployable edge. Raw probabilities were worse than the same-side ask proxy in all four cohorts: model-minus-ask Brier was +0.213 US high, +0.129 global high, +0.741 US low, and +0.238 global low. The formal 20,736-rule US-high run registered two overlapping research-only variants, but both share four holdout trades over three dates, have a -0.396 5% holdout R/R bound, concentrate 50% of trades in one station, and turn negative when the best date is removed. Direct truth audit found all nine relevant venue buckets and matched IEM labels.

A global-low top-bucket YES rule looked positive on snapshot asks, but 95 selections collapsed to three executable $25 tape decisions, all losses; none of ten later viewed rows executed. Seventy-three checkpoints exceeded the 30-second budget. Other-cohort settlement contracts and useful markouts remain unavailable. The verdict is no new shadow and no funded change; preserve the registered versions only for attribution. Detailed evidence is under reports/net-edge/t0024-current-data-opportunity-map/ and reports/discovery/t0024-current-2026-08-13/.
...
- F0B proved that historical grouped model metrics were scored on synthetic ladders centered on the realized high and are invalid for promotion comparison. F3 then found that F0B's hour-bucket selector admitted post-cutoff reports on 4,338/4,364 rows. T0021 found the forward database path repeated the same class of error by admitting the full 14:00 hour. `forecast_fixed_support_exact_cutoff_weather_date_v2` now owns both paths, and diagnostics prove zero post-cutoff rows in each.
...
- Corrected F3 rejects `remaining_heating_hurdle_multinomial_exact_cutoff_v3`. The 45.14% remaining-heating / 54.86% conditioned-HRRR ensemble still improves untouched weather-only scores, but the market-aware combination worsens holdout log loss by `0.00258` and recent log loss by `0.03028`.
- Corrected F4 keeps `asos_upwind_residual_exact_cutoff_v2` rejected. It has only 35.51% eligible coverage on 535 exact-cutoff rows and worsens untouched log loss by `0.12294` and RPS by `0.00659` versus corrected F3, with negative recent results.
- F6 preserves the baseline's selected market, bucket, and side for 18 post-activation rows over 14 dates. F3 is worse than the same-side market by `0.05574` Brier and `0.14519` log loss; uncalibrated diagnostic quote caps lose `0.74` on `6.74` risk; only 2/18 t0 rows have continuously valid, `$25`-fillable tape and their median net edge is `-0.16308`. The curve is diagnostic because no preactivation calibrator was frozen and F3 failed first.
...
16. Preserve the two August 13 mechanically emerged versions as research registry artifacts only. Do not call them newly frozen forward shadows, alter them retrospectively, route them to orders, or infer promotion until post-activation venue, markout, concentration, calibration, and useful-size gates pass.
...
## Audit Log
- 2026-08-13: T0024 completed the current-data four-cohort tournament with no demonstrated deployable edge. All cohorts trailed the ask proxy. Two US-high variants emerged mechanically but fail uncertainty, concentration, best-date, calibration, and markout gates. A global-low cheap-YES snapshot anomaly collapsed to three executable $25 tape rows, all losses, with zero later executions. No new shadow, funded authority, or GOES setting changed.

- 2026-08-13: T0023 now has independent exact-12 and exact-14 F5 arms. The deterministic exact-12 predecessor (`10704409...ca48`) trains only on exact-12 historical rows and improved over HRRR on a 146-date 2025 diagnostic by `-0.06733` log loss and `-0.13697` RPS; this is a conditional control, not market-relative edge. Its GOES version, fingerprint, report directory, calibrator, and untouched clock are isolated from exact-14. Both arms still have zero eligible resolved dates, so no information, execution, or funded authority changed.
...
- 2026-08-13: T0021 found and corrected a second exact-cutoff leak in the F3
  ...
  log-loss gates; corrected F4 remains rejected with only 35.51% coverage.
  F6 then preserved baseline selection across 18 post-activation rows and
  rejected the candidate on selected market-relative skill, preactivation
  calibration, diagnostic quote economics, and useful-size t0 tape evidence.
  The executable decay curve is diagnostic only. Funded authority remains
  paused.
