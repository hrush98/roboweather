...
| Forecast and settlement truth | F0 reconciled the current IEM report maximum to 220/220 venue-winning buckets in a 230-row June audit and froze its source semantics; this establishes a cohort-bounded target proxy, not forecast edge. No new ensemble/spatial source has passed incremental-skill gates. | Backfill provenance/venue labels, then test WeatherNext/NBM, spatial residuals, and observed radiation on identical causal rows. |
