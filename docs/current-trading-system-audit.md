...
| Phase 3 shared tape | Slices 1-4 are implemented. Recorder discovery/reconnect/health/scoped-acceptance defects are repaired and a 1,364-token short live probe passed, but no post-fix first-listing-through-close lifecycle has passed; Slice 4 also lacks one real persisted-quote join. | Install the repaired unit and run one fresh scoped lifecycle probe; continue to fail closed on gaps. |
