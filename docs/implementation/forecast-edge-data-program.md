...
- Run two predeclared lifecycle arms: `d0_exact_12_local` for the strategically earlier test and `d0_exact_14_local` for the scientifically cleaner late control. Both begin source evidence on 2026-08-14, but they have separate predecessor versions, evaluation fingerprints, GOES model versions, generated directories, calibrators, and untouched evidence clocks. Never score 12:00 by time-shifting the 14:00 F3 artifact.
- The exact-12 predecessor is `remaining_heating_hurdle_multinomial_exact_12_local_v1`. It trains its remaining-heating model on 13,152 exact-12 rows across 1,096 dates in 2022-2024, fits a 67.02% remaining-heating / 32.98% HRRR blend on the first 218 dates of 2025, and improves over HRRR on the later 146-date diagnostic by `-0.06733` log loss and `-0.13697` RPS with both date-clustered intervals below zero. Its builder publishes atomically, accepts only byte-identical rebuilds under the same version, and fails on changed output. This establishes an independent conditional forecast control, not market-relative or GOES edge.
- At each exact horizon, derive trailing observed-radiation state without using files first observed after the decision. Condition the challenger on that arm's frozen predecessor forecast and contemporaneous selected-token market probability.
- Freeze broad and cloud-sensitive regimes, surprise thresholds, and abstention levels before outcomes. Report exact selected-token calibration, market-relative log loss, station/regime slices, and an abstention curve separately for each horizon.
- The late contract is `goes_dsr_market_relative_logit_exact_14_local_v1`; the early contract is `goes_dsr_market_relative_logit_exact_12_local_v1`. Each regularized selected-token logit conditions on its horizon-specific predecessor, normalized contemporaneous market distribution, and clear/mixed/cloudy regime controls; its challenger adds radiation surprise plus only predeclared regime interactions. Each arm independently requires 20 resolved calibration dates, freezes exactly its earliest 20 plus a row hash and strictly future activation boundary, then requires another 20 untouched dates. Acceptance requires date-clustered log-loss intervals below zero versus the no-surprise conditional baseline, predecessor, and market; absolute calibration bias at most 0.05; fixed-bin ECE at most 0.10; signed and absolute 0.10/0.20 surprise slices; station/regime diagnostics; and displayed-ask abstention levels of 0.00/0.05/0.10/0.15.
- Displayed asks remain diagnostics, not useful-size execution evidence. If either information arm passes, open a separate cross-pillar executable-ask and t0/+30s/+2m/+5m/+15m edge-decay gate for that exact arm before any trading claim.
...
- 2026-08-13: Added the independently frozen `d0_exact_12_local` F5 arm before any post-activation outcome. Its predecessor uses `remaining_heating_hurdle_multinomial_exact_12_local_v1`, its GOES challenger uses `goes_dsr_market_relative_logit_exact_12_local_v1`, and both its generated directory and calibrator/untouched clock are separate from exact-14. A byte-identical rebuild verified artifact determinism. The predecessor improves over HRRR on a 146-date 2025 diagnostic, but neither F5 horizon has an eligible resolved date or information-edge verdict.
- 2026-08-13: Superseded the readiness-only F5 v1 protocol with `goes_dsr_market_relative_logit_v2` before any eligible outcome existed. The report now freezes exactly the earliest 20 calibration dates and a replay hash behind an explicit strictly future boundary, waits for 20 untouched dates before scoring, and emits the complete clustered comparison, exact-token calibration, signed threshold, station/regime, and abstention contract. The original source evidence clock is unchanged and no edge is established.
- 2026-08-13: Finalized the shared multi-horizon semantics before any eligible outcome by replacing the F3-specific feature/manifest names with horizon-specific predecessor identity. Exact-14 now uses `goes_dsr_market_relative_logit_exact_14_local_v1`; exact-12 retains its own version. This changes no source clock or evidence, but prevents an early arm from masquerading as F3.
