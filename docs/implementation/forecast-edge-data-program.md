...
- The frozen statistical contract is `goes_dsr_market_relative_logit_v2`: a regularized selected-token logit conditioned on F3, the normalized contemporaneous market distribution, and clear/mixed/cloudy regime controls; the challenger adds radiation surprise plus only its predeclared regime interactions. Calibration requires 20 resolved weather dates, then the command freezes exactly the earliest 20, an immutable row hash, and a strictly future untouched activation boundary before any forward score is computed. The untouched gate requires another 20 dates, date-clustered log-loss intervals below zero versus the no-surprise conditional baseline, F3, and market, absolute calibration bias at most 0.05, fixed-bin ECE at most 0.10, signed and absolute 0.10/0.20 surprise slices, station/regime diagnostics, and displayed-ask abstention levels of 0.00/0.05/0.10/0.15. Displayed asks remain diagnostics, not useful-size execution evidence.
