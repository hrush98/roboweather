...
- Added `scripts/market_tape_lifecycle_report.py`, which aggregates all catalog sessions and fails closed on incomplete eligible listing-to-close coverage, operational coverage gaps, collector errors, queue saturation, reconstruction errors, missing lag telemetry, RSS, projected daily disk growth, or absent authoritative listing timestamps. Late/fallback markets and peak lag excursions are reported as exclusions/warnings rather than poisoning otherwise eligible lifecycles.
...
- The operational probe budgets are: at least 12 recorded hours, Gamma listing discovery within 300 seconds, no lifecycle recovery gap over 30 seconds, RSS at or below 1 GiB, queue high-water below capacity, projected raw growth at or below 25 GiB/day, and a 14-day retention projection. Peak receipt lag over 10 seconds is a lifecycle warning; current-session health remains strict, and decision replay never bridges a gap regardless of duration.
...
- Lifecycle acceptance reports every refresh-member market listed inside the selected observation window, then defines the matured subset whose scheduled close is at or before the validation end. Late or fallback-listed markets remain visible but are excluded individually; incomplete eligible markets fail the run. Listed markets whose scheduled close is later than the validation end are `RIGHT_CENSORED`; authoritative pre-window seed markets remain outside the cohort.
...
- Added `deploy/systemd/roboweather-market-tape.service` for ongoing policy-independent research collection after the bounded validation run. It preserves the validated resource/reconnect configuration but intentionally omits the validation-run ID, persisted deadline, and 72-hour runtime cap. The bounded lifecycle unit remains available only for reproducible validation cohorts.
- Added catalog-level advisory locking so only one recorder can write a given catalog, plus TUI control of the continuous user service. The TUI reads health through a read-only catalog connection, displays current validity/resource/discovery failures and journal output, confirms stop/restart actions, and never stops the service when the TUI exits.
- Enabled continuous service `roboweather-market-tape.service` on August 4. Initial session `tape-20260804T152718Z-b341a291` reached `VALID` for 1,474/1,474 tokens, completed discovery, and strictly verified its first rotated partition with queue high-water 1/10,000, about 142 MiB RSS, and zero reconstruction errors. The August 2-to-activation interval is retained as an invalid evidence gap.
...
- [ ] Memory, disk, queue depth, and operational recovery remain within explicit budgets; peak receipt-lag warnings are reviewed.
...
- 2026-08-04: Integrated continuous tape operations into the TUI through user systemd rather than child-process ownership. Added single-writer catalog protection, read-only lightweight health polling, journal visibility, and confirmation-protected stop/restart controls.
- 2026-08-04: Enabled the continuous service and verified fresh full-book coverage plus the first rotated partition. Collection continues under systemd; TUI shutdown does not stop it.
