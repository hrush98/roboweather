...
- Validation sessions persist their run, exact tracked-build, and collector-config fingerprints. Acceptance fails when selected sessions mix or omit any fingerprint. Every initial/scheduled discovery attempt also persists its status and returned token/market membership; failed or incomplete refreshes fail acceptance.
- The lifecycle denominator is every refresh-member market whose listing timestamp falls inside the selected observation window, before authority, discovery-lag, close, or coverage checks. Late, fallback-listed, open, and coverage-incomplete cohort markets remain explicit failures instead of disappearing from the denominator. Authoritative markets listed before the observation window remain background seed markets rather than cohort members.
