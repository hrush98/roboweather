...
- The lifecycle report includes every refresh-member market whose listing timestamp falls inside the selected observation window. Its pass/fail cohort is the matured subset whose scheduled close is at or before the validation end. Late, fallback-listed, or coverage-incomplete matured markets remain explicit failures. Later-closing listings are reported as `RIGHT_CENSORED` and do not fail the matured cohort; authoritative markets listed before the observation window remain background seed markets.
